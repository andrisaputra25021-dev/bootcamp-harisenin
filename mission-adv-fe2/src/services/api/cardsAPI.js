...
export const addCourse = async (newCourse) => {
  ...
  return response.data.id ? response.data : { ...newCourse, id: Date.now() }; // pastikan ada id
};

// UPDATE
export const updateCourse = async (id, updatedCourse) => {
  const response = await axios.put(`${API_URL}/${id}`, updatedCourse);
  return response.data.id ? response.data : { ...updatedCourse, id }; // pastikan return object
};
